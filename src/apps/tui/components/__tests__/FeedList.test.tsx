...
describe('FeedList', () => {
  const createMockFeed = (id: number, rating: number = 0, unreadCount: number = 10): FeedWithUnreadCount => ({
    id,
  ...
  describe('スライディングウィンドウページネーション', () => {
    it('同じレーティングに11件のフィードがある場合、最初の10件のみ表示される', () => {
      ...
      const { lastFrame } = render(
        <FeedList feeds={feeds} selectedIndex={0} />
      );
    ...
    it('11番目のフィードを選択した場合、2-11番目が表示される', () => {
      const feeds = Array.from({ length: 11 }, (_, i) => createMockFeed(i + 1, 3, 10));
      const { lastFrame } = render(
        <FeedList feeds={feeds} selectedIndex={10} />
      );

      const output = lastFrame();
      
      // Feed 1は表示されていない（Feed 10やFeed 11と区別するため正確にマッチ）
      expect(output).not.toMatch(/Feed 1(?!\d)/);
      
      // Feed 2から11までが表示されている
      for (let i = 2; i <= 11; i++) {
    ...
    it('5番目のフィードを選択した場合、1-10番目が表示される', () => {
      ...
      const { lastFrame } = render(
        <FeedList feeds={feeds} selectedIndex={4} />
      );
    ...
    it('最後のフィードを選択した場合、最後の10件が表示される', () => {
      ...
      const { lastFrame } = render(
        <FeedList feeds={feeds} selectedIndex={19} />
      );
  ...
  describe('レーティング別の表示', () => {
    it('異なるレーティングのフィードが混在する場合、現在選択中のレーティングセクションのみ展開される', () => {
      ...
      const { lastFrame } = render(
        <FeedList feeds={feeds} selectedIndex={5} />
      );
    ...
    it('各レーティングセクションで独立してスライディングウィンドウが動作する', () => {
      ...
      const { rerender, lastFrame } = render(
        <FeedList feeds={feeds} selectedIndex={14} />
      );
  ...
  describe('未読件数の表示', () => {
    it('フィードごとの未読件数が正しく表示される', () => {
      const feeds = [
        createMockFeed(1, 3, 5),
        createMockFeed(2, 3, 0),
        createMockFeed(3, 3, 100),
      ];
    ...
    it('総未読件数が正しく表示される', () => {
      const feeds = [
        createMockFeed(1, 5, 10),
        createMockFeed(2, 5, 20),
        createMockFeed(3, 3, 30),
      ];
  ...
  describe('選択状態の表示', () => {
    it('選択されたフィードに選択マーカーが表示される', () => {
      ...
      const { lastFrame } = render(
        <FeedList feeds={feeds} selectedIndex={1} />
      );
  ...
  describe('空の状態', () => {
    it('フィードが登録されていない場合、適切なメッセージが表示される', () => {
      const { lastFrame } = render(
        <FeedList feeds={[]} selectedIndex={0} />
      );
